...
# Definindo função que junta as informações necessárias para o texto do dia em que o progaram é executado.
# Defining function that gathers needed information for the text of the day that the program runs.
def gather_info(url):
    ...
    # Formatando data do dia atual de acordo com parâmetro da div em que o texto se encontra.
    # Formatting current day's date according to the parameter of the div containing the text.
...
# Definindo função que faz o parsing do html recebido e acha a div do texto do dia atual.
# Defining function that parses received html and finds current day's div.
...
# Definindo função que encontra cada tipo de informação para escrever no .txt.
# Defining function that finds each type of information to be written in the .txt file.
...
# Definindo função que escreve o arquivo .txt no diretório atual.
# Defining function that writes the .txt file in the current directory.
...
# Abrindo o arquivo gerado depois de terminada a execução.
# Opening the generated file after the execution ends.
